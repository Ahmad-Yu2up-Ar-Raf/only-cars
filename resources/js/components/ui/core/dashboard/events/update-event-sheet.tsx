...
interface UpdateTaskSheetProps
  ...
  task: EventsSchema | null;
}

export function UpdateEventSheet({ task, ...props }: UpdateTaskSheetProps) {
  ...
  function onSubmit(input: EventsSchema) {
    ...
    startTransition(async () => {
      try {
        ...
        router.post(route('dashboard.events.update', task.id), formData, {
          preserveScroll: true,
          preserveState: true,
          forceFormData: true,
          onBefore: (visit) => {
          ...
          onSuccess: (page) => {
            ...
            toast.success('Event updated successfully', { id: 'update-toast' });
            form.reset();
    ...
    return (
      <Sheet {...props} modal={true}>
        <SheetContent className="flex flex-col gap-6 overflow-y-scroll">
          ...
          <TaskForm<EventsSchema> form={form} onSubmit={onSubmit} isPending={loading} initialFiles={task!.files ? task!.files   : undefined}  fileUploadRef={fileUploadRef}   >
