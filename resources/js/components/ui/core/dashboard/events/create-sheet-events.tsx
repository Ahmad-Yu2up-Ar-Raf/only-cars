...
import { TaskForm } from "./events-form";
import { EventsSchema, eventsSchema } from "@/lib/validations/validations";
...
export function CreateEventsSheet({ ...props}: type) {
...
function onSubmit(input: EventsSchema) {
  ...
  startTransition(() => {
    ...
    router.post(route(`dashboard.events.store`), input, { 
      preserveScroll: true,
      preserveState: true,
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen} modal={true} >
      ...
      <SheetContent className="flex flex-col gap-6 overflow-y-scroll ">
        <SheetHeader className="text-left sm:px-6 space-y-1 bg-background z-30  sticky top-0   p-4 border-b  ">
          <SheetTitle className=" text-lg">Add New <Button type="button"   variant={"outline"} className=" ml-2  px-2.5 text-base capitalize">Events</Button> </SheetTitle>
          <SheetDescription className=" sr-only">
            Fill in the details below to create a new task
          </SheetDescription>
        </SheetHeader>
        <TaskForm type="events" isPending={loading} form={form}  fileUploadRef={fileUploadRef}  onSubmit={onSubmit}>
...
return(
   ...
   <DrawerTrigger asChild className={cn(props.trigger == false && "sr-only" )} >
      ...
      <DrawerContent className="flex flex-col  ">
        ...
        <DrawerTitle className=" text-xl">Add New <Button type="button"   variant={"outline"} className=" ml-2  px-2.5 text-base">Events</Button> </DrawerTitle>
