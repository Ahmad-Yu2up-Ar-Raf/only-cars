...
import { Calendar, Camera, EllipsisIcon, Eye, EyeOff, User2Icon, Users2Icon } from "lucide-react"
...
import { CreateGallerySheet } from "../create-sheet-gallery";
...
import { TasksTableActionBar } from "./gallery-table-action-bar";
import { UpdateGallerySheet } from  "../update-gallery-sheet";
...
export function GalleryDataTable({Gallery, filters , className, PaginatedData}: componentsProps) {
...
const [searchTerm, setSearchTerm] = React.useState(filters.search);
  ...
  const [openModal, setOpenModal] = React.useState(false)
    ...
    () =>
      debounce((search: string) => {
        router.get(route(`dashboard.gallery.index`), {
          search: search
        }, {
          preserveState: true,
          preserveScroll: true
  ...
  const AllIds: number[] = Gallery.map(item => item.id!);
...
console.log(taskId)
          toast.loading("Gallery deleting...",  { id: "gallery-delete" });
      router.delete(route(`dashboard.gallery.destroy`, id), {
        ...
        onSuccess: () => {
          toast.success("Gallery deleted successfully",  { id: "gallery-delete" });
          setOpenModal(false);
          router.reload(); 
        },
        onError: (errors: any) => {
          console.error("Delete error:", errors);
          toast.error(errors?.message || "Failed to delete the gallery" , { id: "gallery-delete" });
        },
        onFinish: () => {
          setProcessing(false);
        }
      ...
      toast.error("An unexpected error occurred",  { id: "gallery-delete" });
      setProcessing(false);
    }
  };
...
const [isAnyPending, setIsAnypending] = React.useState<boolean>(false);
  ...
  const onTaskDelete = React.useCallback(() => {
    ...
    toast.loading("Deleting data...", { id: "gallery-delete" });
    
    startTransition(async () => {
      try {
        router.delete(route(`dashboard.gallery.destroy`, selectedIds), {
        ...
        toast.error("Failed to delete items", { id: "gallery-delete" });
        setCurrentAction(null);
      }
    });
  }, [Gallery, selectedIds, pathNames]);
const [currentGallery , setcurrentGallery ] = React.useState<(GallerySchema) | null>(null);
    const [openUpdate, setOpenUpdate] = React.useState(false)
  const handleEdit = (merchandise: GallerySchema) => {
    setcurrentGallery(merchandise);
    setOpenUpdate(true);
  };
 const handleUpdateClose = (open: boolean) => {
      ...
      startTransition(async () => {
        try {
          ...
          const formData = {
              ...
              router.post(route(`dashboard.gallery.status`, selectedIds), formData, {
                preserveScroll: true,
                preserveState: true,
                forceFormData: true,
                onBefore: (visit) => {
  ...
  if(Gallery.length == 0 && filters.search == "")
  ...
  <EmptyState
          icons={[Camera]}
          title={`No Gallery data yet`}
          description={`Start by adding your first gallery`}
          action={{
            label: `Add gallery`,
    ...
    <Table>
      <TableCaption className=" sr-only">A list of your recent gallery.</TableCaption>
      ...
      <TableBody>
        {Gallery.length > 0 ?  Gallery.map((gallery) =>{ 
          ...
          <TableRow key={gallery.id}>
            <TableCell className="font-medium sticky right-0 ">
              ...
              checked={selectedIds.includes(gallery.id!)}
                      onCheckedChange={() => HandleSelect(gallery.id!)}
        ...
        {gallery.visibility}
            ...
            <DropdownMenuContent align="end" className="w-40">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem
             onSelect={() => handleDelete(gallery.id!)}
              >
                Delete
    ...
    {selectedIds.length} of {Gallery.length} row(s) selected.
      </div>
