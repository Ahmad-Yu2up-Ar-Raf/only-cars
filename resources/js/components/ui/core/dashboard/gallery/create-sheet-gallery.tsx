...
export function CreateGallerySheet({ ...props}: type) {
...
console.log(input)
    toast.loading("Gallery creating....", {
      id: "create-gallery"
    });
  ...
  startTransition(() => {
    ...
    router.post(route(`dashboard.gallery.store`), input, { 
      preserveScroll: true,
      preserveState: true,
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen} modal={true} >
      ...
      <SheetContent className="flex flex-col gap-6 overflow-y-scroll ">
        <SheetHeader className="text-left sm:px-6 space-y-1 bg-background z-30  sticky top-0   p-4 border-b  ">
          <SheetTitle className=" text-lg">Add New <Button type="button"   variant={"outline"} className=" ml-2  px-2.5 text-base capitalize">Gallery</Button> </SheetTitle>
          <SheetDescription className=" sr-only">
            Fill in the details below to create a new task
          </SheetDescription>
        </SheetHeader>
        <TaskForm type="gallery" isPending={loading} form={form}  fileUploadRef={fileUploadRef}  onSubmit={onSubmit}>
          <SheetFooter className="gap-3 px-3 py-4 w-full flex-row justify-end  flex  border-t sm:space-x-0">
            ...
            <Button disabled={loading} type="submit" className="w-fit dark:bg-primary !pointer-gallery-auto  dark:text-primary-foreground  bg-primary text-primary-foreground " size={"sm"}>
              {loading && <Loader className="animate-spin" />}
              Add
            </Button>
          </SheetFooter>
...
return(
   ...
   <DrawerTrigger asChild className={cn(props.trigger == false && "sr-only" )} >
      ...
      <DrawerContent className="flex flex-col  ">
        ...
        <DrawerTitle className=" text-xl">Add New <Button type="button"   variant={"outline"} className=" ml-2  px-2.5 text-base">Gallery</Button> </DrawerTitle>
        ...
        <DrawerFooter className="gap-3 px-3 py-4 w-full flex-row justify-end  flex  border-t sm:space-x-0">
             <DrawerClose disabled={loading} asChild onClick={() => form.reset()}>
                          ...
                          <Button type="submit"  disabled={loading} className="w-fit  !pointer-gallery-auto  dark:bg-primary  dark:text-primary-foreground  bg-primary text-primary-foreground " size={"sm"}>
                            {loading && <Loader className="animate-spin" />}
                            Add
                          </Button>
        </DrawerFooter>
